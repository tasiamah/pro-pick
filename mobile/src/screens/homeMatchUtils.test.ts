...
describe('selectComingUpMatches', () => {
  ...
  it('keeps upcoming matches within the one-week horizon, sorted by kickoff', () => {
    const started = createMatch(1, new Date(2026, 5, 24, 15, 0).toISOString());
    const soon = createMatch(2, new Date(2026, 5, 24, 20, 0).toISOString());
    const laterThisWeek = createMatch(3, new Date(2026, 5, 29, 18, 0).toISOString());
    const beyondHorizon = createMatch(4, new Date(2026, 6, 3, 18, 0).toISOString());

    const result = selectComingUpMatches(
      [beyondHorizon, laterThisWeek, started, soon],
      now,
      anchor,
    );

    // Started match dropped, beyond-horizon (>7 days) dropped, rest sorted.
  ...
  it('anchors the horizon on a future slate during a fixture gap', () => {
    ...
    const withinFromAnchor = createMatch(1, new Date(2026, 6, 5, 18, 0).toISOString());
    const beyondFromAnchor = createMatch(2, new Date(2026, 6, 12, 18, 0).toISOString());
